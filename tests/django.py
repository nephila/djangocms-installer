...
class TestDjango(BaseTestClass):
    ...
    def test_create_project(self):
        ...
        config_data = config.parse(['--db=postgres://user:pwd@host/dbname',
                                    '-q', '-p'+self.project_dir, 'test_project'])
        install.requirements(config_data.requirements)
        django.create_project(config_data)
        self.assertTrue(os.path.exists(os.path.join(self.project_dir, 'test_project')))
    ...
    def test_patch(self):
        ...
        project = __import__('%s' % config_data.project_name,
